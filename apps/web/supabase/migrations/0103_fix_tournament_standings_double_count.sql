-- ============================================================
-- 0103 — Fix double-count en get_tournament_standings (doubles/2v2)
-- ============================================================
-- BUG (reportado 2026-07-31):
--   En torneos "todos contra todos" 2v2, el leaderboard mostraba
--   PF y PC duplicados. Ej: partida 106–33 →
--     ganadores mostraban 212 PF cada uno (esperado 106)
--     perdedores mostraban 66  PC cada uno (esperado 33 desde el otro lado)
--
-- CAUSA:
--   La RPC introducida en 0101 calculaba team_score y opp_score con:
--     select sum(mp2.score) from match_players mp2 where team = mp.team
--   Pero `match_players.score` está DENORMALIZADO por team, no por jugador:
--   syncMatchScores (live-match.ts) hace `update ... where team = X` y
--   escribe el mismo score total en TODAS las filas del team. Con 2v2
--   ambos compañeros tienen 106 → sum = 212 (doblado).
--
--   El comentario en 0051_dual_leaderboard.sql línea 178 lo advertía:
--     "match_players.score está duplicado en doubles".
--
-- FIX:
--   Cambiar `sum(mp2.score)` → `max(mp2.score)` en los dos lateral joins.
--   Todos los rows del mismo team tienen el mismo valor denormalizado,
--   así que MAX (o MIN, o cualquier fila) devuelve el team_score correcto
--   sin sumar entre compañeros.
--
-- INVARIANTE (lección):
--   NUNCA usar `sum(match_players.score)` particionado por team dentro de
--   un mismo match. Ese patrón dobla el score en 2v2. Si necesitas el
--   team_score, usa MAX(mp.score) o léelo de match_rounds (source of truth).
--   Si necesitas el total de un jugador a lo largo de N partidas, sí puedes
--   sumar `mp.score` agrupando por user_id (cada partida es una fila).
--
-- BACKFILL:
--   No hace falta. get_tournament_standings es una función pura sobre
--   match_players/match_rounds. Deploy → siguiente query devuelve valores
--   correctos.
-- ============================================================
...
-- IMPORTANTE: usamos MAX(mp2.score), NO SUM. match_players.score está
-- denormalizado por team (ver comentario del archivo). SUM dobla en 2v2.
per_match_player as (
  select
    m.id                                                       as match_id,
    m.target_points,
...
-- ============================================================
-- 1. Torneo con 1 partida 2v2 confirmada 106–33:
--    SELECT user_id, pf, pc FROM get_tournament_standings('<uuid>');
--    → ganadores: pf=106, pc=33
--    → perdedores: pf=33, pc=106
--
-- 2. Torneo con múltiples partidas: pf de un jugador = suma de scores
--    de su pareja en cada partida en la que jugó (no × 2).
-- ============================================================
