...
import { closeContinuousLeague } from "@/lib/continuous-league-actions";
...
export function CloseContinuousLeagueDialog({ tournamentId, onClose }: Props) {
  ...
  async function handleConfirm() {
    ...
    setPending(true);
    setError(null);
    const res = await closeContinuousLeague(tournamentId);
    if (!res.ok) {
      setError(res.error);
      setPending(false);
      return;
  ...
  return (
    ...
    >
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="close-continuous-league-title"
      ...
      >
        <div className="flex items-center gap-2 text-danger">
          ...
          <h2 id="close-continuous-league-title" className="text-lg font-semibold">Cerrar polla</h2>
        </div>
